...
# Get IPv4 address of wlan0 interface
ip_addr=$(ip -4 addr show wlan0 2>/dev/null | grep inet | awk '{print $2}' | cut -d/ -f1 || echo "N/A")

# Get WiFi SSID
wifi_ssid=$(iw dev wlan0 link 2>/dev/null | awk '/SSID/ {print $2}')
